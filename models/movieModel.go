...
type Genre struct {
	...
	GenreName string `bson:"genre_name" json:"genre_name" validate:"required, min=1,max=100"`
}

type Ranking struct {
	RankingValue int    `bson:"ranking_value" json:"ranking_value" validate:"required"`
	RankingName  string `bson:"ranking_name" json:"ranking_name" validate:"required,min=1,max=100"`
}
...
type Movie struct {
	...
	Genres      []Genre       `bson:"genres" json:"genres" validate:"required,dive"`
